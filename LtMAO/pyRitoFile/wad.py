...
# Safe imports / Mocking dependencies
try: 
    import pyzstd
    from xxhash import xxh64, xxh3_64
except ImportError:
    # Define fallback or dummy classes if not present
    # This ensures the file can be imported even if we can't use detailed WAD writing features
    class PyZstdMock:
        def decompress(self, data): raise ImportError("pyzstd not installed")
        def compress(self, data): raise ImportError("pyzstd not installed")
    pyzstd = PyZstdMock()
    
    def xxh64(data): 
        class Digest:
            def intdigest(self): return 0
        return Digest()
    ...
    def xxh3_64(data):
        class Digest:
            def intdigest(self): return 0
        return Digest()
    # print('Warning: pyRitoFile.wad failed to import pyzstd, xxhash. WAD reading/writing might be limited.')
